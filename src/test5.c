...
int funct_open_file(int argc, char **argv, FILE *fname) {
  for (int i = 1; i < argc;
       i++)  // start from 1, not 0. depends of arg counter.
  {
    printf("arg string is: %s\n", argv[i]);
    fname =
        fopen(argv[i], "r");  // is trying to open a string name (sorting thru)
    printf("current file name is: %p\n", fname);
    if (fname !=
        NULL)  // if it couldn't open, it means that it have read a flag
