...
// soo, "int main" is basic stuff, and "return 0" is not rly needed, but u
// should type it anyway for consistency sake.
...
//         fgets(buffer, 255, fp); // soo it is "fgets("write to" | "string
//         length" | "stream (from where)")" printf("%s", buffer); if (feof(fp)
//         == 1) break;
//         }
//     }
