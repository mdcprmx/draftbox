...
// int getopt(int argument_counter, char* const argument_values[], const char *
// optstring);
...
int main(int arg_cntr, char* arg_val[]) {
  ...
  // first example, "a:bCd" (in this case "flag a" will have a parameter, number
  // or etc)
  ...
  //////////////
  // also, if flag may, or may not have a parameter (number) u must type "::"
  // after supposed char. first example, "a::" second example, "a::bcdesxa"
  // third example, "ab::c::des::x"

  opterr = 0;  // yeah it looks like an error, but it actually turns off error
               // msgs! nice.
