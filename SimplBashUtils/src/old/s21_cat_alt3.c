...
int main(int argc, char *argv[]) {
                 ...
                 // executable name btw) do:
  {
    ...
    filename = funct_open_file(
        argc,
        argv);  // tries to open a file // btw, FIX THIS! IT MUST LOOK FOR
                // FILENAME, not just take argv[1], cuz there could be a flag
...
void scenario_file_print(int filename) {
  ...
  while ((bytes_length = read(filename, buffer,
                              BUFFER_SIZE)))  // function "read" returns number
                                              // of bytes that were read.
  {
    write(STDOUT_FILENO, buffer,
          bytes_length);  // so it writes the exact number of bytes that func
                          // "read" returned.
  }
  close(filename);
}
